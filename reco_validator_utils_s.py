...
def analysis_of_reco2(rc_reco_df, non_rc_reco_df, speciality_reco_df, path_reco_analysis_s):
    '''Function will analyse the reco dataframes.
...
def error_in_log2(log_file_path):
    '''This function will find if there is error in log file and return the error if present.
...
def check_reco_type_and_user_type_are_consistent(rc_reco, non_rc_reco, path_graph_reco_user_list \
    ...
    if len(set(non_rc_random_reco_user) - set(random_user_list)) != 0:
        send_mail('non rc random reco was generated for non random users')
